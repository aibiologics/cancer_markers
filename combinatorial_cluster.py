...
This script is a combinatorial algo. Combinations are assessed by clustering them to create a composite variable.
...
def compute_fitness(df, variables, cluster_num, cluster_method, multi, test):
    """ Estimate the coefficient of a composite variable made of combination of variables (unsupervised_clustering)."""
    x = df[variables]
    model = AgglomerativeClustering(n_clusters=cluster_num, linkage=cluster_method)
    model.fit(x)
    if multi:
        comb_df = df[["Survival", "Status", "agerecoded", "Tstagen", "Nstagen", "EMVI"]].assign(cluster=model.labels_)
        cph1 = CoxPHFitter().fit(comb_df, 'Survival', 'Status', formula="C(cluster)+ agerecoded+EMVI+Nstagen+Tstagen")
        return float(cph1.hazard_ratios_[:-4].values.sum())
    else:
        comb_df = df[["Survival", "Status"]].assign(cluster=model.labels_)
        if test == 'logrank':
            result = pairwise_logrank_test(comb_df['Survival'], comb_df['cluster'], comb_df['Status'])
            return float(result.summary['-log2(p)'].values[0:4].sum())
        else:
            cph1 = CoxPHFitter().fit(comb_df, 'Survival', 'Status', formula='cluster')
            return float(cph1.hazard_ratios_.values.sum())


def test_combination(df, variables, total_model, model, drop_missing_rate, interpolation_method, train_test,
                     cluster_num, cluster_method, multi, test):
    ...
    if total_model:
        ...
        fitness = compute_fitness(df, variables, cluster_num, cluster_method, multi, test)
    return variables, fitness


def compare_comb_stepwise_global(df, variables, step, total_model, model,
                                 drop_missing_rate, interpolation_method, train_test, cluster_num, cluster_method,
                                 multi, test):
    ...
    for x in range(len(variables) - 1, len(variables) - step, -1):
        ...
        for combination in population:
            current_solution = test_combination(df, list(combination), total_model, model, drop_missing_rate,
                                                interpolation_method, train_test, cluster_num, cluster_method,
                                                multi, test)
...
def optimal_comb_stepwise_global(df, variables, step=4, total_model=True, model=None, drop_missing_rate=1.0,
                                 interpolation_method='linear', generation_num=20, train_test=False, sac_rate=1.0,
                                 cluster_num=5, cluster_method='ward', multi=True, test='cox'):
    ...
    generation_num:int, default 1000
    ...
    best_solution = compare_comb_stepwise_global(df, variables, step, total_model, model,
                                                 drop_missing_rate, interpolation_method, train_test, cluster_num,
                                                 cluster_method, multi, test)
    ...
    for i in range(generation_num):
        current_solution = compare_comb_stepwise_global(df, best_solution[0], step, total_model, model,
                                                        drop_missing_rate, interpolation_method, train_test,
                                                        cluster_num,
                                                        cluster_method, multi, test)
...
def compare_comb_gradient(df, variables, total_model, model, drop_missing_rate, interpolation_method, train_test,
                          cluster_num, cluster_method, multi, test):
    ...
    for combination in population:
        ...
        current_comb = test_combination(df, list(combination), total_model, model, drop_missing_rate,
                                        interpolation_method, train_test, cluster_num, cluster_method, multi, test)
...
def optimal_comb_gradient(df, variables, total_model=True, model=None, drop_missing_rate=1.0,
                          interpolation_method='linear', generation_num=25, train_test=False, sac_rate=1.0,
                          cluster_num=5, cluster_method='ward', multi=True, test='cox'):
    ...
    if not model:
        model = CoxPHSurvivalAnalysis()
    best_solution = compare_comb_gradient(df, variables, total_model, model, drop_missing_rate,
                                          interpolation_method, train_test, cluster_num, cluster_method, multi, test)
    logging.info('[iteration 0] Best solution: %s', best_solution)
    for i in range(generation_num):
        current_solution = compare_comb_gradient(df, list(best_solution[0]), total_model, model,
                                                 drop_missing_rate, interpolation_method, train_test, cluster_num,
                                                 cluster_method, multi, test)
...
if __name__ == '__main__':
    ...
    optimal_comb_stepwise_global(data, markers_subset, total_model=False, sac_rate=0.80)
