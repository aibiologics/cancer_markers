...
through random mutation of first child, and finally picking up children with the best fitness
...
def generation(df, initial_population, variables,composite_compute, combinations_num, parent_selection_method,
               children_num, mut_rate, total_model, model, drop_missing_rate, interpolation_method, train_test, multi,
               test):
    """ generate random combinations, select best as parents, generate children and perform mutations and select the
    best combination to to the next generation."""
...
def optimal_generation(df, initial_population, variables, combinations_num, children_num, composite_compute='cluster',
                       parent_selection_method='roulette', total_model=True, model=None, train_test=False,
                       mut_rate=0.2, drop_missing_rate=1.0, interpolation_method='linear', iteration=1000, multi=True,
                       test='cox', sac_rate=1.0):
    """ random combinations, select best combinations, intersection, mutations, find fittest combination which is taken
     forward for next generation.
   Parameters
   ----------
   df: Dataframe
   ...
   test: str, default cox.
    ...
    fittest_child = generation(df, initial_population, variables, composite_compute, combinations_num,
                               parent_selection_method, children_num, mut_rate,total_model, model, drop_missing_rate,
                               interpolation_method, train_test, multi, test)
    ...
    for i in range(iteration):
        current_child = generation(df, initial_population, fittest_child[0], composite_compute, combinations_num,
                                   parent_selection_method, children_num, mut_rate, total_model, model,
...
if __name__ == '__main__':
    ...
    optimal_generation(data, sig_markers, markers_subset, 30, 12, composite_compute='linear', total_model=False,
                       train_test=False, sac_rate=0.9)
